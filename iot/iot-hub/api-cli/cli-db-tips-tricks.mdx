---
title: Discovering IoT Hub CLI Database Route Tips & Tricks
description: This page details advanced PostgreSQL usage in IoT database route
excerpt: |
  This page shows examples of data manipulation on the PostgreSQL side:
  - converting payloads to explicit types,
  - decoding and extracting JSON fields,
...
2. Try by publishing some access events
  ```bash
  sleep 5 # wait a little for the route to start
  mosquitto_pub \
    -h $(jq -r '.endpoint' hub.json) \
...
3. Enter the follwing commands to check that everything is as expected:
  ```bash
